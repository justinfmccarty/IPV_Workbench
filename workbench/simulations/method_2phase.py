...
def change_plastic_material(
    og_material_fp, blk_material_fp, line_idx, item_idx, new_name
):
...
def build_cmd_oconv(radiance_project_dir, radiance_surface_key, step):
    ...
    object_material_file = os.path.join(
        radiance_surface_dir, "model", "scene", "envelope.mat"
    )
    black_object_material_file = os.path.join(
        radiance_surface_dir, "model", "scene", "envelope.blk"
    )
    ...
    black_object_file = os.path.join(
        radiance_surface_dir, "model", "scene", "envelope_black.rad"
    )
    ...
    glazing_material_file = os.path.join(
        radiance_surface_dir, "model", "aperture", "aperture.mat"
    )
    if os.path.exists(glazing_material_file):
        ...
        glazing_file = os.path.join(
            radiance_surface_dir, "model", "aperture", "aperture.rad"
        )
        black_glazing_file = os.path.join(
            radiance_surface_dir, "model", "aperture", "aperture_black.rad"
        )
...
def build_cmd_rfluxmtx(
    ...
    grid_file = glob.glob(os.path.join(radiance_surface_dir, "model", "grid", "*.pts"))[
        0
    ]
...
def build_cmd_rcalc(cal="reinsrc.cal"):
    ...
    if "win32" in sys.platform:
        cmd += [
            ...
            "Rbin=recno",
            "-o",
            r""" "solar source sun 0 0 4 ${Dx} ${Dy} ${Dz} 0.533" """,
        ]
    else:
        cmd += [
            "-e",
...
def run_2phase_dds(project, year=2099):
    ...
    if pathlib.Path(scenario_tmy).suffix == ".epw":
        cmd_epw2wea, output_wea = build_cmd_epw2wea(
            radiance_project_dir, radiance_surface_key, scenario_tmy
        )
        # run command
        proc = subprocess.run(
            cmd_epw2wea, check=True, input=None, stdout=subprocess.PIPE
        )
        # filter wea
    ...
    for n, step in enumerate(["total", "direct"]):
        ...
        ## oconv
        print("     - oconv")
        cmd_oconv, output = build_cmd_oconv(
            radiance_project_dir, radiance_surface_key, step
        )
        ...
        ## rfluxmtx
        print("     - rfluxmtx")
        cmd_rfluxmtx, output_file, input_file = build_cmd_rfluxmtx(
            radiance_project_dir,
            radiance_surface_key,
            skyglow_template_file,
            step,
            use_accelerad=project.irradiance_use_accelerad,
            n_workers=n_workers,
        ...
        ## gendaymtx
        print("     - gendaymtx")
        cmd_gendaymtx, output_file = build_cmd_gendaymtx(
            radiance_project_dir, radiance_surface_key, output_wea, step
        )
        with open(output_file, "w") as file:
            proc = subprocess.run(
                cmd_gendaymtx, stderr=subprocess.PIPE, stdout=file, check=True
            )

        ## dctimestep and rmtxop
        print("     - dctimestep | rmtxop")
        # First command: dctimestep matrices/dc/illum.mtx skyVectors/NYC.smx
        cmd_dctimestep = build_cmd_dctimestep(
            radiance_project_dir, radiance_surface_key, step
        )
        proc_dctimestep = subprocess.Popen(cmd_dctimestep, stdout=subprocess.PIPE)
        # Second command: rmtxop -fa -t -c 47.4 119.9 11.6 -
        cmd_rmtxop, output_file = build_cmd_rmtxop(
            radiance_project_dir, radiance_surface_key, step
        )
        proc_rmtxop = subprocess.Popen(
            cmd_rmtxop,
            stdin=proc_dctimestep.stdout,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    ...
    ## Run rcontrib
    print("     - rcontrib")
    cmd_rcontrib, output_file, input_file = build_cmd_rcontrib(
        radiance_project_dir,
        radiance_surface_key,
        use_accelerad=project.irradiance_use_accelerad,
        n_workers=n_workers,
    ...
    proc_rmtxop = subprocess.Popen(
        cmd_rmtxop, stdin=proc_dctimestep.stdout, stdout=subprocess.PIPE
    )
    with open(output_file, "wb") as file:
        file.write(proc_rmtxop.communicate()[0])
